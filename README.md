# 🎯 LeetCode Practice

170 problems across 17 patterns to master algorithms and data structures.

---

## 📋 Progress

- [ ] **[Sorting Algorithms](./sorting-algorithms/)** (7/10) ✅✅✅✅✅✅✅⬜⬜⬜
- [ ] **[Two Pointers](./two-pointer/)** (3/10) ✅✅✅⬜⬜⬜⬜⬜⬜⬜
- [ ] **[Sliding Window](./sliding-window/)** (0/10) ⬜⬜⬜⬜⬜⬜⬜⬜⬜⬜
- [ ] **[Fast & Slow Pointers](./fast-slow-pointers/)** (0/10) ⬜⬜⬜⬜⬜⬜⬜⬜⬜⬜
- [ ] **[Binary Search](./binary-search/)** (0/10) ⬜⬜⬜⬜⬜⬜⬜⬜⬜⬜
- [ ] **[Hash Tables](./hash-tables/)** (0/10) ⬜⬜⬜⬜⬜⬜⬜⬜⬜⬜
- [ ] **[Linked Lists](./linked-lists/)** (0/10) ⬜⬜⬜⬜⬜⬜⬜⬜⬜⬜
- [ ] **[Stacks & Queues](./stacks-queues/)** (0/10) ⬜⬜⬜⬜⬜⬜⬜⬜⬜⬜
- [ ] **[Tree Fundamentals](./tree-fundamentals/)** (0/10) ⬜⬜⬜⬜⬜⬜⬜⬜⬜⬜
- [ ] **[Trees (DFS/BFS)](./trees/)** (0/10) ⬜⬜⬜⬜⬜⬜⬜⬜⬜⬜
- [ ] **[Binary Search Trees](./binary-search-trees/)** (0/10) ⬜⬜⬜⬜⬜⬜⬜⬜⬜⬜
- [ ] **[Heaps](./heaps/)** (0/10) ⬜⬜⬜⬜⬜⬜⬜⬜⬜⬜
- [ ] **[Graphs](./graphs/)** (0/10) ⬜⬜⬜⬜⬜⬜⬜⬜⬜⬜
- [ ] **[Dynamic Programming](./dynamic-programming/)** (0/10) ⬜⬜⬜⬜⬜⬜⬜⬜⬜⬜
- [ ] **[Backtracking](./backtracking/)** (0/10) ⬜⬜⬜⬜⬜⬜⬜⬜⬜⬜
- [ ] **[Greedy](./greedy/)** (0/10) ⬜⬜⬜⬜⬜⬜⬜⬜⬜⬜
- [ ] **[Bit Manipulation](./bit-manipulation/)** (0/10) ⬜⬜⬜⬜⬜⬜⬜⬜⬜⬜

**Total: 10/170** (5.88%)

---

## 🚀 How to Use

### Structure
```
category/
  problem-name/
    index.ts           # Run this
    test-cases.ts      # Test cases
    solution-1.ts      # Your implementations
    solution-2.ts
    README.md          # Auto-generated results
```

### Run
```bash
bun two-pointer/1-palindrome/index.ts
```

### Results
- Console output with tables
- README.md auto-generated with benchmark results
- Compare multiple solutions automatically

---

## 📚 Categories

Each category README has 10 problems with examples and constraints.

**Start here:** Sorting Algorithms, Tree Fundamentals, Two Pointers, or Hash Tables
